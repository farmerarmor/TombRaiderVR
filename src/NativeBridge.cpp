...
void Log(const char* fmt, ...) {
    ...
    FILE* f{}; if (!fopen_s(&f,"TombRaiderVR.log","a")) {
...
struct Bridge {
    ...
    bool Init(ID3D11Device* dev) {
        ...
        strcpy_s(ci.applicationInfo.applicationName,"TombRaiderVR native stereo");
        ...
        wchar_t config[MAX_PATH]{};GetFullPathNameW(L"TombRaiderVR.ini",MAX_PATH,config,nullptr);
    ...
    bool EnsureSwapchain(const D3D11_TEXTURE2D_DESC& d,UINT h) {
        auto outputFormat=PresentationFormat(d.Format);
        if(swapchain && width==d.Width && height==h && format==outputFormat)return true;
        DestroySwapchain();
        uint32_t n=0;
        ...
        if(std::find(formats.begin(),formats.end(),int64_t(outputFormat))==formats.end()) {
            Log("Presentation format %d for source %d is unsupported by runtime",outputFormat,d.Format); return false;
        }
        ...
        ci.format=outputFormat; ci.sampleCount=1; ci.width=d.Width; ci.height=h; ci.faceCount=1; ci.arraySize=2; ci.mipCount=1;
        ...
        width=d.Width; height=h; format=outputFormat;
        Log("Native pair swapchain: %ux%u per eye, arraySize=2 format=%d sourceFormat=%d images=%u",width,height,format,d.Format,n);
        return true;
    }
    void Capture(ID3D11Texture2D* pair,UINT h,uint64_t id) {
        ...
        for(UINT eye=0;eye<2;eye++) {
            char name[180]; sprintf_s(name,"TombRaiderVR-captures/pair-%llu-%s.bmp",id,eye?"right":"left");
    ...
    void Frame(ID3D11Texture2D* pair,UINT h,bool swap,bool capture,bool recenter) {
        ...
        if(capture && history.Count()) {
            char folder[180];sprintf_s(folder,"TombRaiderVR-captures/history-%lu-%llu",GetCurrentProcessId(),sourceFrame);
            ...
            if(Good(xrAcquireSwapchainImage(swapchain,&ai,&index),"xrAcquireSwapchainImage")) {
                ...
                if(wait==XR_SUCCESS && index<images.size()) {
                    ...
                    if(Good(xrReleaseSwapchainImage(swapchain,&ri),"xrReleaseSwapchainImage")) {
                        // Queued scenes can carry different tracking samples in
                        // one completed native pair (mode 2). Keep showing both
                        // current eyes using the pair's first recorded pose;
                        // suppressing the layer caused TAB-menu black flashes.
                        bool tracked=(renderInfo.mode==1 || renderInfo.mode==2) && renderInfo.eyeMask==3 && renderInfo.tracking.valid;
                        if(renderInfo.mode==2) {
                            static uint64_t mixedPairs=0;
                            ++mixedPairs;
                            if(mixedPairs==1 || capture)Log("Mixed scene-pose pair displayed: count=%llu sourceFrame=%llu pose=%llu eyeMask=%u",mixedPairs,sourceFrame,renderInfo.tracking.id,renderInfo.eyeMask);
                        }
                        if(tracked) {
